...
import { FolderOpen, Upload, Trash2, Download, X, FileText, BookText, CalendarRange } from 'lucide-react'
...
interface DokumenItem {
    ...
    semester: string; tahun_ajaran: string; created_at: string
}

const TABS = [
    { key: 'prota', label: 'Prota', icon: BookText, desc: 'Program Tahunan' },
    { key: 'prosem', label: 'Prosem', icon: CalendarRange, desc: 'Program Semester' },
] as const
    ...
    return (
        <div>
            <div className="page-header">
                <div>
                    <h1 className="page-title"><FolderOpen size={28} /> Prota & Prosem</h1>
                    <p className="page-subtitle">Kelola Program Tahunan dan Program Semester</p>
                </div>
                <button className="btn btn-primary" onClick={() => setShowModal(true)}>
                    <Upload size={16} /> Upload {currentTabInfo.label}
                </button>
            </div>
                    ...
                    return (
                        <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                            className="card" style={{
                                flex: 1, cursor: 'pointer', textAlign: 'center', padding: '1rem',
                                border: isActive ? '2px solid var(--primary-500)' : '1px solid var(--neutral-100)',
                                background: isActive ? 'var(--primary-50)' : 'white',
                                transition: 'all 0.2s ease',
                            }}>
                            <Icon size={24} style={{ color: isActive ? 'var(--primary-600)' : 'var(--neutral-400)', marginBottom: '0.5rem' }} />
                            <div style={{ fontWeight: 700, fontSize: '0.95rem', color: isActive ? 'var(--primary-700)' : 'var(--neutral-700)' }}>{tab.label}</div>
                            <div style={{ fontSize: '0.7rem', color: isActive ? 'var(--primary-500)' : 'var(--neutral-400)', marginTop: '0.125rem' }}>{tab.desc}</div>
                        </button>
                    )
                })}
            </div>
            ...
            {loading ? (
                ...
                <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
                    <FolderOpen size={48} style={{ color: 'var(--neutral-300)', marginBottom: '1rem' }} />
                    <p style={{ color: 'var(--neutral-500)' }}>Belum ada {currentTabInfo.label}</p>
                </div>
            ) : (
                    ...
                    <div className="modal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            <div className="form-group">
                                ...
                                <input className="form-input" placeholder={`Masukkan judul ${currentTabInfo.label}`}
